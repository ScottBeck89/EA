...
public class InputManager : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if(input2D.magnitude == 0 && GetComponent<Rigidbody>().velocity.magnitude > 0) {
			GetComponent<Rigidbody>().AddRelativeForce(-(GetComponent<Rigidbody>().velocity.normalized * dampingFactor));
		} else {
			//This gives an immediate feedback in movement.  Instant acceleration clamped to maxSpeed.
			GetComponent<Rigidbody>().velocity = Vector3.ClampMagnitude(input2D * maxSpeed, maxSpeed * speedPercentage);
		}
